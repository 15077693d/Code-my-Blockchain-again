...
class Block {
    ...
    getBData() {
        return new Promise((resolve, reject) => {
            if(this.height===0){
                resolve({message:"This is genesis block",code:200,data:{}})
            }
            // Getting the encoded data saved in the Block
            const encodedData = this.body 
            // Decoding the data to retrieve the JSON representation of the object
            const jsonBody = hex2ascii(encodedData)
            // Parse the data to an object to be retrieve.
            const objectBody = JSON.parse(jsonBody)
            // Resolve with the data if the object isn't the Genesis block
            if(objectBody){
                resolve({message:"success getting data",code:200,data:objectBody})
            }else{
                reject({message:"there is no data.",code:404,data:objectBody})
            }
        })
    }
}
